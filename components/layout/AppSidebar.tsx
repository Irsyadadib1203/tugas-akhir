...
export const AppSidebar = ({ collapsed, onToggle }: AppSidebarProps) => {
  ...
  return (
    <>
      {/* ===================== */}
      {/* DESKTOP — Sidebar kiri (sama seperti sebelumnya) */}
      {/* ===================== */}
      <aside className={cn(
        "hidden md:flex fixed left-0 top-0 h-screen bg-sidebar flex-col z-40 transition-all duration-300",
        collapsed ? "w-20" : "w-64"
      )}>
        {/* Logo */}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-sidebar border-t border-sidebar-border">
        <ul className="flex items-center">
          ...
          {/* Status indikator di pojok kanan bottom bar */}
          <li className="px-4 flex items-center gap-2">
            <span className={cn(
              "w-2 h-2 rounded-full flex-shrink-0",
              isOnline ? "bg-success animate-pulse" : "bg-destructive"
            )} />
